    ...
    public class ReviewController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Id,Content,Rating,CreatedAtUtc,MovieId")] Review review)
        {
            if (ModelState.IsValid)
            {
                ...
                // ÖZELLIK 1: Kullanıcının bu film için zaten review'u var mı kontrol et
                var existingReview = await _context.Reviews
                    .FirstOrDefaultAsync(r => r.UserId == userId && r.MovieId == review.MovieId);

                if (existingReview != null && !User.IsInRole("Admin"))
                {
                    ModelState.AddModelError("", "You have already reviewed this movie. You can edit your existing review.");
        ...
        public async Task<IActionResult> Edit(int id, [Bind("Id,Content,Rating,CreatedAtUtc,MovieId")] Review review)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    // ÖZELLIK 2: Review güncellendiğinde UpdatedAtUtc'yi set et
